...
const mapToFrontend = (item: any): NarrationRecord => ({
  id: item.id,
  fip_id: item.fipId || '',
...
export const narrationService = {
  getAll: async (filters?: Record<string, any>): Promise<NarrationRecord[]> => {
    ...
    const response = await api.get<ApiResponse<any[]>>('/v1/config/narrations', {
  ...
  getById: async (id: string): Promise<NarrationRecord> => {
    const response = await api.get<ApiResponse<any>>(`/v1/config/narrations/${id}`);
  ...
  create: async (data: NarrationRecordFormData & { fipIds: string[] }): Promise<NarrationRecord[]> => {
    ...
    const response = await api.post<ApiResponse<any[]>>('/v1/config/narrations', backendPayload);
    return (response.data.data || []).map(mapToFrontend);
  },
  ...
  update: async (id: string, data: Partial<NarrationRecordFormData>): Promise<NarrationRecord> => {
    ...
    const response = await api.put<ApiResponse<any>>(`/v1/config/narrations/${id}`, backendPayload);
    return mapToFrontend(response.data.data);
  },
  ...
  getNarrationOptions: async (fipId: string, transactionType: string): Promise<NarrationRecord[]> => {
    const response = await api.get<ApiResponse<any[]>>('/v1/config/narrations/options', {
      params: {
        fipId,
        transactionType,
      },
